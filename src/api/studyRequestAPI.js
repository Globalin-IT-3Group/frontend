...
class StudyRequestApi extends BaseApi {
  ...
  async create({ studyRecruitId, message }) {
    // { studyRecruitId, message } 객체를 그대로 전달
  ...
  async getMyRequests() {
    ...
    return res.data; // StudyRequestResponse[] 형태
  }
  ...
  async getRequestsByRecruit(studyRecruitId) {
  ...
  async updateRequestStatus(requestId, status) {
    return this.fetcher.patch(`/study-request/${requestId}/status`, null, {
      params: { status }, // status는 "ACCEPTED" or "REJECTED"
    });
  }
}
